...
public class OptionConverter {
	...
	public static Level toLevel(String value, Level defaultValue) {
		...
		try {
			...
			LogLog.warn("custom level class [" + clazz + "]" + " does not have a class function toLevel(String, Level)",
					e);
		...
		if (className != null) {
			try {
				...
				if (!superClass.isAssignableFrom(classObj)) {
					LogLog.error("A \"" + className + "\" object is not assignable to a \"" + superClass.getName()
							+ "\" variable.");
	 ...
	 * @param clazz
	 *            The class name, of the log4j configurator which will parse the
	 *            <code>inputStream</code>. This must be a subclass of
	 *            {@link Configurator}, or null. If this value is null then a
